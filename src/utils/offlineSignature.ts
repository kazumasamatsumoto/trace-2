...
import { PayloadForOfflineSignature } from '@/types/PayloadForOfflineSignature';
  ...
  export const offlineSignature = async (
    ...
    const clientSignedTx = CosignatureTransaction.signTransactionPayload(client, payloadForOfflineSignature.signedPayload, generationHash);    
    let signedPayload = payloadForOfflineSignature.signedPayload + clientSignedTx.version.toHex() + clientSignedTx.signerPublicKey + clientSignedTx.signature;

    const recreatedTx = TransactionMapping.createFromPayload(payloadForOfflineSignature.signedPayload);
    ...
    const signedTx = new SignedTransaction(signedPayload, payloadForOfflineSignature.signedHash, adminPublicKey, recreatedTx.type, recreatedTx.networkType);
